...
function Analytics() {
  ...
  const dailySeries = useMemo(
    () => buildDailySeries(recentEntries),
    [recentEntries],
  );
  const weeklyScoreSeries = useMemo(
    () => buildWeeklyScoreSeries(recentEntries),
    [recentEntries],
  );
  ...
  return (
    <section className="space-y-7">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
            Recharts render directly from localStorage logs with no external
            API.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatsCard
          label="Avg score"
          value={`${summary.avgScore}/10`}
          detail="All-time"
        />
        <StatsCard
          label="Avg study"
          value={`${summary.avgStudyHours}h`}
        ...
        <StatsCard
          label="Avg Instagram"
          value={`${summary.avgInstagramMinutes}m`}
          detail="Per logged day"
          accent="rose"
        />
      </div>
